...
import { handleCreateMatch } from "@/lib/actions/admin/match-action";
...
export default function CreateMatchForm() {
    ...
    const [form, setForm] = useState({
        seriesName: "", matchNumber: "", matchType: "international", format: "ODI",
        team1Name: "", team1Short: "", team2Name: "", team2Short: "",
        stadiumName: "", city: "", country: "",
        scheduledDate: "", scheduledTime: "",
        isInternational: true, isFeatured: false,
    });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const response = await handleCreateMatch(payload);
        setLoading(false);
            ...
            <Section title="Series Information">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <Field label="Series Name" required>
                        <input name="seriesName" value={form.seriesName} onChange={handleChange}
                            placeholder="e.g. India tour of Australia 2025" className={inputClass} required />
                    </Field>
                    <Field label="Match Number" required>
                        <input name="matchNumber" value={form.matchNumber} onChange={handleChange}
                            placeholder="e.g. 1st ODI, Final" className={inputClass} required />
                    </Field>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-3">
                        ...
                        <Field label="Full Name" required>
                            <input name="team1Name" value={form.team1Name} onChange={handleChange}
                                placeholder="e.g. India" className={inputClass} required />
                        </Field>
                        <Field label="Short Name (max 6)" required>
                            <input name="team1Short" value={form.team1Short} onChange={handleChange}
                                placeholder="e.g. IND" maxLength={6} className={inputClass} required />
                        </Field>
                    </div>
                    <div className="space-y-3">
                        <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">Team 2</p>
                        <Field label="Full Name" required>
                            <input name="team2Name" value={form.team2Name} onChange={handleChange}
                                placeholder="e.g. Australia" className={inputClass} required />
                        </Field>
                        <Field label="Short Name (max 6)" required>
                            <input name="team2Short" value={form.team2Short} onChange={handleChange}
                                placeholder="e.g. AUS" maxLength={6} className={inputClass} required />
                        </Field>
                    </div>
                </div>
            </Section>
            ...
            <Section title="Venue">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <Field label="Stadium Name" required>
                        <input name="stadiumName" value={form.stadiumName} onChange={handleChange}
                            placeholder="e.g. Melbourne Cricket Ground" className={inputClass} required />
                    </Field>
                    <Field label="City" required>
                        <input name="city" value={form.city} onChange={handleChange}
                            placeholder="e.g. Melbourne" className={inputClass} required />
                    </Field>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <Field label="Display Time (UTC)" required>
                        <input name="scheduledTime" value={form.scheduledTime} onChange={handleChange}
                            placeholder="e.g. 14:30" className={inputClass} required />
                    </Field>
                </div>
            </Section>
